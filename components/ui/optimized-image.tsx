...
const getDefaultDimensions = (
  ...
  const imageDimensions: Record<string, { width: number; height: number }> = {
    ...
    "/images/programs/community.svg": { width: 800, height: 600 },
  };
...
export function OptimizedImage({
  ...
  const finalWidth =
    width || defaultDimensions?.width || (fill ? undefined : 1200);
  const finalHeight =
    height || defaultDimensions?.height || (fill ? undefined : 800);
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === "development" && fill) {
      if (!style || (!style.width && !style.height)) {
        console.warn(
          "OptimizedImage: When using fill, parent element must have position relative and defined dimensions"
        );
      }
    }
  }, [fill, style]);
