...
export function AnimatedHeading({ title, subtitle, align = "left", className, subtitleClassName, size = "md", underline = false, }) {
    ...
    return (<div ref={ref} className={cn("mb-8", alignClasses[align], className)}>
      <h2 className={cn("font-bold tracking-tight", sizeClasses[size])}>
        <motion.span className="inline-block" initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }} transition={{ duration: 0.6, ease: [0.215, 0.61, 0.355, 1] }}>
          {title}
        </motion.span>
