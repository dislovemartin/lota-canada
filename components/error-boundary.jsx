...
function DefaultFallback({ error, resetErrorBoundary, }) {
    ...
    return (<div className="flex flex-col items-center justify-center min-h-[400px] p-6 text-center">
      ...
      {process.env.NODE_ENV === "development" && (<div className="mt-8 p-4 bg-muted rounded-md text-left overflow-auto max-w-full">
...
export function ErrorBoundary({ children, fallbackRender, }) {
    return (<ReactErrorBoundary fallbackRender={fallbackRender || DefaultFallback} onError={(error) => {
            // Log the error to analytics
            trackEvent("error_boundary_triggered", {
                category: "error",
                label: error.message,
