...
const directors = [
    ...
    },
    {
        ...
        education: "Educated at Korea University in Business Administration and complemented by a tenure at the South Korean President's Office",
        experience: "Daniel's international experience spans across the globe, having previously contributed to facilitating global dialogues at the G20 Summit, Nuclear Security Summit, and the UN General Assembly.",
    ...
    },
    {
        ...
        mission: "Beyond facilitating property transactions, her mission extends to guiding individuals towards their entrepreneurial dreams. Armed with versatile skills in marketing, event planning, fundraising, and mentorship, she is committed to empowering others to navigate the dynamic business landscape with confidence.",
    ...
    },
    {
        ...
        expertise: "As for entrepreneurial support, Robin specializes in providing bridge financing and private loans to address the funding challenges faced by businesses and entrepreneurs. He maintains strong relationships with a broad spectrum of investors and venture capital funds, offering essential financing pathways and practical fundraising advice.",
...
export default function BoardPage() {
    ...
    const executiveTeam = directors.filter((d) => ["CEO", "COO", "CTO", "CFO", "President"].includes(d.position));
    ...
    return (<div className="container-wide mx-auto py-16">
      <AnimatedHeading title="BOARD OF DIRECTORS" subtitle="Meet the leadership team guiding the Leaders of Tomorrow Association." align="center" underline/>

      <SectionDivider className="mb-16"/>

      {/* All Directors in a single grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 max-w-7xl mx-auto">
        {directors.map((director, index) => (<DirectorCard key={director.id} director={director} index={index}/>))}
      </div>
    </div>);
}
function DirectorCard({ director, index }) {
    ...
    return (<motion.div ref={ref} initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }} transition={{ duration: 0.6, delay: index * 0.1 }} className="flex flex-col items-center h-full bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow duration-300">
      <div className="relative w-full aspect-square mb-4 overflow-hidden rounded-lg shadow-md group">
        <Image src={director.image || "/placeholder.svg"} alt={director.name} fill className="object-cover object-center transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"/>
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
          <div className="flex space-x-4">
            <a href={director.linkedin} className="bg-white/90 p-2 rounded-full hover:bg-white transition-colors duration-200" aria-label={`${director.name}'s LinkedIn`} target="_blank" rel="noopener noreferrer">
              <Linkedin className="h-5 w-5 text-primary"/>
            </a>
            <a href={`mailto:${director.email}`} className="bg-white/90 p-2 rounded-full hover:bg-white transition-colors duration-200" aria-label={`Email ${director.name}`}>
      ...
      <a href={`#bio-${director.id}`} className="mt-auto text-xs font-medium text-primary hover:underline">
        View Full Bio
      </a>

      {/* Bio Modal */}
      <div id={`bio-${director.id}`} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 opacity-0 pointer-events-none transition-opacity duration-300 target:opacity-100 target:pointer-events-auto" onClick={(e) => {
            if (e.target === e.currentTarget) {
                window.location.hash = "";
            }
        }}>
        <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-2xl font-bold">{director.name}</h3>
              <p className="text-primary font-medium">{director.position}</p>
            </div>
            <a href="#" className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" aria-label="Close">
              <X className="h-6 w-6"/>
            </a>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-center mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <div className="flex space-x-4">
                <a href={director.linkedin} className="bg-primary/10 p-2 rounded-full hover:bg-primary/20 transition-colors duration-200" aria-label={`${director.name}'s LinkedIn`} target="_blank" rel="noopener noreferrer">
                  <Linkedin className="h-5 w-5 text-primary"/>
                </a>
                <a href={`mailto:${director.email}`} className="bg-primary/10 p-2 rounded-full hover:bg-primary/20 transition-colors duration-200" aria-label={`Email ${director.name}`}>
                  <Mail className="h-5 w-5 text-primary"/>
                </a>
              </div>
            </div>
